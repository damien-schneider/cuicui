...
export function AddToCart({ initialCartCount }: { initialCartCount: number }) {
	...
	return (
		<button
			className="relative w-full items-center space-x-2 rounded-lg bg-vercel-blue px-3 py-1 font-medium text-sm text-white hover:bg-vercel-blue/90 disabled:text-white/70"
			onClick={addToCart}
			disabled={isPending}
		>
			Add to Cart
			{isPending ? (
				<div className="absolute top-1.5 right-2" role="status">
